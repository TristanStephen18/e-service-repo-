...
class WildlifeFarmLarge extends StatefulWidget {
  final String name;
  ...
  @override
  _WildlifeFarmLargeState createState() => _WildlifeFarmLargeState();
}

class _WildlifeFarmLargeState extends State<WildlifeFarmLarge> {
    ...
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .set({
            ...
            'type': 'Wildlife Farm Permit (Large)',
            'authority': 'RED',
          });
  ...
  Future<void> _submitFiles() async {
    ...
    if (management != null) {
      filesToUpload['Management'] = management!;
    }
    ...
    if (indigenous != null) {
      filesToUpload['Letter of Commitment'] = indigenous!;
    }
    if (financialPlan != null) {
      filesToUpload['Financial Plan'] = financialPlan!;
    }
    if (lantTitle != null) {
      filesToUpload['Land Title'] = lantTitle!;
    }

    if (article != null) {
      filesToUpload['Articles of Incorporation'] = article!;
    }
    if (priorClearance != null) {
      filesToUpload['Prior Clearance'] = priorClearance!;
    }
    if (bir != null) {
      filesToUpload['BIR Registration'] = bir!;
    }
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildFilePicker(
                  '2. Copy of the Certificate of Registration from the appropriate Govenrment AgenciesManagement and breeding plan in accordance with the attached outline',
                  management,
                  (file) => setState(() => management = file),
                ),
                _buildFilePicker(
                ...
                _buildFilePicker(
                  '4. Photo of the existing facility (for those converted to wildlife farm) or sketch/ development plan of proposed facility ',
                  photo,
                  (file) => setState(() => photo = file),
                ),
                _buildFilePicker(
                  '5. In case of indigenous threatened species, letter of commitment to simultaneously undertake conservation breeding and propose measures on rehabilitation and/or protection, where appropriate, as may be determined by the RWMC',
                  indigenous,
                  (file) => setState(() => indigenous = file),
                ),

                _buildFilePicker(
                  '6. Certified copy of Land Title or Lease Contract for the facility',
                  lantTitle,
                  (file) => setState(() => lantTitle = file),
                ),
                _buildFilePicker(
                  '7. Financial plan showing financial capability to go into breeding',
                  financialPlan,
                  (file) => setState(() => financialPlan = file),
                ),
                _buildFilePicker(
                  '8. Photocopy of Articles of Incorporation, in case of corporation',
                  article,
                  (file) => setState(() => article = file),
                ),
                _buildFilePicker(
                  '9. Prior clearance from the affected communities, i.e. concerned LGUs, recognized head of the indigenous people in accordance with RA 8371, or Protected Area Management Board',
                  priorClearance,
                  (file) => setState(() => priorClearance = file),
                ),

                _buildFilePicker(
                  '10. Copy of BIR registration as exporter, if applicant will engage export',
                  bir,
                  (file) => setState(() => bir = file),
                ),
                ...
                _buildFeeRow('Permit Fee', 5000.00),

                const Divider(thickness: 1.2),
                _buildFeeRow('TOTAL', 5500.00, isTotal: true),
